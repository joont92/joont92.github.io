...
아래는 전달받은 파라미터를 Level 타입으로 변환해주는 컨버터이다.  
```java
...
첫째로 직접 클래스를 만들고 GenericConversionService를 상속받은 뒤, addConverter() 메서드로 컨버터들을 등록하는 방식이다. 이후 빈으로 등록한다.  

둘째는 추가할 컨버터들을 빈으로 등록해두고 ConversionServiceFactoryBean을 이용해서 컨버터들이 추가된 GenericConversionService를 빈으로 등록하는 방식이다.  
...
하지만 WebDataBinder는 하나의 ConversionService 타입 오브젝트만 허용한다.  

컨버터도 PropertyEditor처럼 WebBindingInitializer를 이용해 일괄등록 할 수 있다.  
