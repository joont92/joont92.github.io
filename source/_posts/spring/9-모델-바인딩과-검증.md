...
스프링은 등록된 에러 코드를 `messages.properties`와 같은 파일에서 찾아와 에러 메세지로 활용한다.  
보통 메세지 파일에는 아래와 같이 등록되어 있을 것이다.  
```
name.required=이름은 필수로 입력하셔야 합니다.
```
근데 이렇게 간단하게 사용되는 것은 아니고, 스프링의 `MessageCodeResolver`를 통해 좀 더 세분화되어 사용된다.  

우리가 등록한 에러코드 `name.required`는 스프링의 `DefaultMessageCodeResolver`를 거친 후 아래와 같이 4가지 에러코드로 확장된다.  
1. 에러코드.모델이름.필드이름 : name.required.user.name
2. 에러코드.필드이름 : name.required.name
3. 에러코드.타입이름 : name.required.User
4. 에러코드 : name.required

이 4가지 에러코드는 위에서부터 우선순위를 가지며 메세지를 찾는데 사용된다.  
즉 메세지 파일이 아래와 같다면,  
```
name.required.user.name=무언가 잘못된 메세지
name.required=이름은 필수로 입력하셔야 합니다.
```
이 방식을 알지 못할 시, 의도하지 않은 메세지가 생성될 수 있으니 주의하여야 한다.  

개인적으로 좀 혼란스러운 방식이라고 생각한다..  
게다가 검증방식(rejectValue, reject, 타입 오류 등, JSR-303)에 따라 에러코드가 각자 다르게 생성되니 사용할 떄 항상 주의해야 한다.  

## MessageSource
에러코드에 해당하는 메세지를 생성할 떄는 `MessageSource`라는 것을 사용한다.   이는 디폴트로 등록되지 않으니 스프링의 빈으로 직접 등록해줘야 한다.  
