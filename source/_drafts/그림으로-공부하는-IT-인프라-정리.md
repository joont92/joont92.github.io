...
- 병렬 처리에서는 분기점, 직렬화 구간, 합류점이 병목 지점이 되기 쉽다
    - 분기나 합류하는 지점에서 오버헤드가 발생한다
- 병렬 처리는 구조가 복잡하여 설계가 어렵다
- 이런 단점을 감안해도 효과가 있을 경우 병렬화를 하는 것이 좋다
- 아파치는 멀티 프로세스, 톰캣은 멀티 스레드로 동작한다
- 병렬처리를 할때는 CPU 코어 개수도 고려해야 한다
    - 1코어에 병렬처리를 해봤자 소용이 없다

- 비동기의 본질은 병렬이다? NIO의 본질은 병렬이다?
- 비동기는 아래의 2가지를 확인하고 진행해야한다
    - 비동기로 요구한 처리가 끝나지 않은 상태에서 다음 처리를 진행해도 되는가?
    - 비동기로 요구한 처리라 끝났는지 확인할 필요가 있는가?

- run-queue : CPU 처리를 기다리고 있는 프로세스 행렬
    - 보통 run-queue 에 쌓인 프로세스를 코어수로 나누어서 1이라면 문제없다고 본다
- CPU 코어가 여러개일 경우 run-queue 에 있는 프로세스를 비어있는 CPU에 할당하며 빠르게 처리한다
- DISK 는 CPU 처럼 여러개가 될 수 없으므로 프로세스가 1줄로 서서 기다려야만 한다

- 배타적 제어란 내가 건드리고 있을 떄 다른 사람이 건드리지 못하게 하는 것을 말한다
- 병렬처리와 같은 환경에서 주로 필요하다
- 이 부분은 병목지점이 되기 쉽다
- 각 처리가 서로 관계없이 동작하는 경우에는 배타적 제어가 필요없지만, 대부분은 공유 데이터를 이용하며, 직렬로 처리해야 할 부분이 존재한다
- 정말 필요한 곳에서만 배타적 제어를 사용하며 병렬처리를 하면 CPU를 효율적으로 사용해 처리속도를 높일 수 있다
- 일반적으로 프로세스가 CPU를 기다릴떄는 컨텍스트 스위칭이 일어나고, 스핀락을 이용하면 컨텍스트 스위칭이 일어나지 않는다?

- 상태를 가지면 과거의 정보를 가져올 수 있어서 세분화된 처리가 가능하다는 장점이 있으나, 복잡해진다는 단점이 있다(있을수 없는 상태 등)
- 상태를 가지지 않으면 간단하다는 장점이 있어서 성능이나 안정성을 쉽게 향상시킬 수 있으나, 세분화된 처리가 힘들다는 점이 있다
- 프로세스는 CPU에 처리되기 위해 상태를 가진다
- HTTP는 상태를 가지지 않지만, 상태가 필요한 구간이 있으므로 세션이나 쿠키를 사용한다

- 고정 길이는 공간이 낭비될 수 있으나 탐색 속도가 빠르다
- 가변 길이는 공간을 효율적으로 사용할 수 있으나 탐색 속도가 느리다
- 디스크 저장에서 고정 길이를 사용한다
- 네트워크 전송에는 가변 길이를 사용한다

- 해시테이블은 삽입/삭제가 빠른 linked list 안에 탐색이 빠른 array list 를 넣는 구조이다
- 오라클은 SQL 캐시에 해시테이블을 사용한다
    - 캐시는 계속 변하기 때문에 Linked List로 관리한다
    - 캐시에 hit 되어서 찾는 SQL 구문을 빨리 읽기 위해 Array List 를 사용한다
    - 뭐 이정도 까지만..

- 데이터 정리 방식을 데이터 구조, 찾는 방법을 탐색 알고리즘 이라고 한다
- 데이터 정리(저장)와 찾는 방법은 뗄레야 뗄 수 없는 구조이다

- 얼마나 적은 노력으로 원하는 데이터를 추출할 수 있느냐가 관건인데, 인덱스는 그냥 수단일 뿐이다
- 풀테이블 스캔을 하면 모든 데이터를 메모리에 올린 후 데이터를 조사한다

- HDD DB는 B 트리가 적합하고, 메모리 DB는 T 트리(이진)가 적합하다
- B 트리는 등호 검색과 범위 검색에 강하고, 해시 테이블은 등호 검색에 좀 더 강하다
    - 키를 해시 함수를 사용하여 고정된 크기의 해시값으로 변환하여 저장한다
- 모든 데이터가 필요한 경우에는 선형 탐색(풀테이블 스캔)이 적합하다

- 캐시는 사용 빈도가 높은 데이터를 고속으로 액세스 할 수 있는 위치에 두는 것을 말한다
- 캐시는 재사용을 전제로 한다
- 클라이언트와 웹 서버 사이에 캐시서버를 두거나 CDN을 사용하는 경우가 많다
- 참조 빈도가 높고, 손실되도 문제가 없는 데이터는 캐시를 사용하는 것이 적합하다
- 데이터 갱신 빈도가 높거나 너무 큰 데이터의 경우 캐시를 사용하는 것이 부적합하다
- 어떤 데이터를 캐시하는것이 효율적인지 검토하고 선택해야 한다
- 시스템 가동 직후에는 캐시 데이터가 없기 때문에 원하는 성능이 나오지 않을 수 있다
- 캐시를 두면 결국 계층이 늘어나는 것이기 때문에 데이터 불일치 문제의 용의자가 늘어나는 상황이 발생한다
- 캐시 데이터가 손실되었을 경우의 복구 순서를 설계시에 확립해야 한다

- CPU가 정기적으로 폴링하여 디스크나 입출력 장치의 요청을 확인하기에는 텀이 맞지 않으므로, 끼어들기(이벤트)를 사용하는 것이 더 효과적이다
    - 폴링 주기가 길면 반응속도가 느릴것이고, 폴링 주기가 짧으면 CPU 부하가 커질것이다

- 폴링은 정기적으로 질의하여 상태나 요구사항을 확인하고 처리하는 것을 말한다
- 이벤트 주도(끼어들기)는 폴링의 반대이다
- 일정 간격으로 처리를 실행하면 좋은 곳이나, 감시가 필요한곳(시스템이 죽으면 자발적으로 상태를 전달할 수 없으므로)에 사용하면 좋다
- 입력에 따라 실행 내용을 변경하는 곳이나 처리 우선순위가 있는 곳에서 폴링을 사용하는 것은 부적합하다
- 네트워크를 경유한 폴링시에 처리 지연시간을 줄이기 위해 폴링 주기를 너무 짧게하면 트래픽 양이 증가하고 리소스 소비가 늘어나므로 주의가 필요하다
